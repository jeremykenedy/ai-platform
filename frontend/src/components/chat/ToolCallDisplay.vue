  ...
  switch (props.toolCall.status) {
    case 'success':
      return {
        ...
        badgeClasses: 'bg-green-50 text-green-700 ring-green-600/20 dark:bg-green-950/30 dark:text-green-400 dark:ring-green-400/20',
      }
    case 'error':
      return {
        label: 'Error',
        ...
        badgeClasses: 'bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-950/30 dark:text-red-400 dark:ring-red-400/20',
      }
    default:
      return {
        label: 'Pending',
        ...
        badgeClasses: 'bg-yellow-50 text-yellow-700 ring-yellow-600/20 dark:bg-yellow-950/30 dark:text-yellow-400 dark:ring-yellow-400/20',
      }
  }
})
...
<template>
  <div class="my-2 overflow-hidden rounded-lg border border-border bg-muted/30 text-sm dark:border-border dark:bg-muted/20">
    <!-- Header -->
    <button
      class="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-muted/50 dark:hover:bg-muted/30"
      @click="expanded = !expanded"
    >
      <component :is="expanded ? ChevronDown : ChevronRight" class="h-3.5 w-3.5 shrink-0 text-muted-foreground dark:text-muted-foreground" />
      ...
      <span v-if="durationLabel" class="ml-1 text-xs text-muted-foreground dark:text-muted-foreground">
        {{ durationLabel }}
      </span>
    </button>
    ...
    <div v-if="expanded" class="border-t border-border dark:border-border">
      <!-- Input -->
      <div class="px-3 py-2">
        <p class="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground dark:text-muted-foreground">
          Input
        </p>
        <pre class="overflow-x-auto rounded-md bg-background p-2 text-xs text-foreground dark:bg-background dark:text-foreground"><code>{{ formattedInput }}</code></pre>
      </div>

      <!-- Output -->
      <div v-if="toolCall.output !== undefined && toolCall.output !== null" class="border-t border-border px-3 py-2 dark:border-border">
        <p class="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground dark:text-muted-foreground">
          Output
        </p>
        <pre
          class="overflow-x-auto rounded-md bg-background p-2 text-xs dark:bg-background"
          :class="toolCall.status === 'error' ? 'text-red-600 dark:text-red-400' : 'text-foreground dark:text-foreground'"
