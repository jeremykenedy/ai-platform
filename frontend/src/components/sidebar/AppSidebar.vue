<script setup>
  ...
  const hasConversations = computed(() =>
    Object.values(filteredGrouped.value).some((l) => l.length > 0)
  )
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto py-2">
        <template v-if="hasConversations">
          <div v-for="(list, group) in filteredGrouped" :key="group" class="mb-2">
            <p
              :class="
                cn(
        ...
        <div v-else-if="conversations.isLoading" class="flex items-center justify-center py-8">
          <div
            class="h-5 w-5 animate-spin rounded-full border-2 border-primary border-t-transparent"
          />
        </div>

        <div v-else class="flex flex-col items-center justify-center gap-2 py-12 text-center px-4">
          ...
          <button
            v-if="!searchQuery"
            :class="
              cn('text-sm font-medium transition-colors', 'text-primary hover:text-primary/80')
            "
            @click="newChat"
          >
            Start a new chat
      ...
      <div :class="cn('border-t border-gray-200 dark:border-gray-800 p-2')">
        <div
          :class="
            cn(
