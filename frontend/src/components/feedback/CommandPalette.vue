<script setup>
  ...
  function onKeydown(e) {
    ...
    if (e.key === 'ArrowUp') {
      e.preventDefault()
      activeIndex.value =
        (activeIndex.value - 1 + (allItems.value.length || 1)) % (allItems.value.length || 1)
      return
    }
        ...
        >
          ...
          <div class="max-h-96 overflow-y-auto py-2">
            <template v-if="!isEmpty">
              <div v-for="(group, gi) in filteredGroups" :key="group.label" class="mb-1">
                <p
                  class="px-4 pb-1 pt-2 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500"
                >
                  {{ group.label }}
                </p>
                <button
          ...
          <!-- Footer hint -->
          <div
            class="flex items-center gap-4 border-t border-gray-200 px-4 py-2 dark:border-gray-700"
          >
