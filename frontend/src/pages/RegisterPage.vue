...
<template>
  <AuthLayout title="Create your account" subtitle="Complete your registration below">
    ...
    <div v-if="!token" class="space-y-4">
      <div class="flex flex-col items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-5 py-6 text-center dark:border-amber-900/50 dark:bg-amber-950/30">
        <ShieldAlert class="h-8 w-8 text-amber-500 dark:text-amber-400" />
        <div>
          <p class="font-semibold text-amber-800 dark:text-amber-300">Registration is invite-only</p>
    ...
    <form v-else class="space-y-4" @submit.prevent="handleSubmit">
      <!-- Invite token badge -->
      <div class="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-700 dark:border-green-900/50 dark:bg-green-950/30 dark:text-green-400">
