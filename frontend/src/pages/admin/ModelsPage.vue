  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex items-center justify-between">
        <h2 class="text-sm font-semibold text-blue-900 dark:text-blue-100">Pull Model from Ollama</h2>
        <button @click="showPullPanel = false" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200">
          <X class="h-4 w-4" />
        </button>
      </div>
    ...
    <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
      ...
      <div v-if="loadingRunning" class="flex items-center gap-2 px-5 py-4 text-sm text-gray-500 dark:text-gray-400">
        <RefreshCw class="h-3 w-3 animate-spin" />
        Loading...
      </div>
      <div v-else-if="runningModels.length === 0" class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400">
        No models currently loaded
      </div>
      <ul v-else class="divide-y divide-gray-100 dark:divide-gray-800">
        <li
          v-for="m in runningModels"
          :key="m.name"
          class="flex items-center gap-4 px-5 py-3"
        >
          <div class="flex h-2 w-2 shrink-0 rounded-full bg-green-500" />
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{{ m.name }}</p>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Provider</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Name</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden lg:table-cell">Display Name</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden xl:table-cell">Parameters</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden xl:table-cell">Context</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden lg:table-cell">Capabilities</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Source</th>
              <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Active</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              <td class="px-5 py-3 text-gray-600 dark:text-gray-400 capitalize">{{ m.provider }}</td>
              <td class="px-5 py-3 font-mono text-xs text-gray-900 dark:text-gray-100">{{ m.name }}</td>
              <td class="px-5 py-3 text-gray-700 dark:text-gray-300 hidden lg:table-cell">{{ m.display_name ?? m.name }}</td>
              <td class="px-5 py-3 text-gray-600 dark:text-gray-400 hidden xl:table-cell">{{ m.parameters ?? '-' }}</td>
              ...
              <td class="px-5 py-3 hidden lg:table-cell">
                <div class="flex flex-wrap gap-1">
                  <span
                    ...
                    :class="capabilityBadge[cap] ?? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'"
                  >
                    {{ cap }}
                  </span>
                </div>
