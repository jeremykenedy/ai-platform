...
async function fetchActiveModels() {
  try {
    const response = await api.get('/admin/models', { params: { active: 1 } })
    activeModels.value = response?.data?.data ?? response?.data ?? []
  } catch {
    // Non-critical
  }
}
...
async function startJob() {
  if (!jobForm.value.dataset_id || !jobForm.value.base_model_id || !jobForm.value.output_model_name.trim()) {
  ...
  try {
    ...
    jobForm.value = { dataset_id: '', base_model_id: '', output_model_name: '', config: '{\n  "epochs": 3,\n  "learning_rate": 0.0001,\n  "batch_size": 4\n}' }
...
<template>
  <div class="space-y-5">
    ...
    <!-- Tabs -->
    <div class="flex gap-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 p-1 w-fit">
      <button
    ...
    <div v-if="activeTab === 'datasets'" class="space-y-5">
      ...
      <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 space-y-4">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Dataset Name</label>
            <input
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Format</label>
            <select
      ...
      <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
        ...
        <div v-else-if="datasets.length === 0" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                ...
                <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Format</th>
                <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden sm:table-cell">Rows</th>
                <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden sm:table-cell">Size</th>
                <th class="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400 hidden md:table-cell">Created</th>
                ...
                <td class="px-5 py-3">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p v-if="d.description" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ d.description }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-3">
                  <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 capitalize">
                    {{ d.format }}
                  </span>
                </td>
    ...
    <div v-else class="space-y-5">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <button @click="showStartForm = false" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200">
            <X class="h-4 w-4" />
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Dataset <span class="text-red-500">*</span></label>
            <select
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Base Model <span class="text-red-500">*</span></label>
            <input
          ...
          <div class="sm:col-span-2">
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Output Model Name <span class="text-red-500">*</span></label>
            <input
          ...
          <div class="sm:col-span-2">
            <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Config (JSON)</label>
            <textarea
      ...
      <div v-else-if="jobs.length === 0" class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-5 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <h3 class="font-semibold text-gray-900 dark:text-gray-50 truncate">{{ job.output_model_name }}</h3>
                <span
          ...
          <div v-if="job.status === 'running' || (job.progress > 0 && job.progress < 100)" class="mt-4">
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div class="relative flex w-full max-w-2xl flex-col rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-2xl max-h-[80vh]">
        <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 px-6 py-4 shrink-0">
          <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-50 truncate">{{ logModal.title }}</h2>
          <button
        ...
        <div class="flex-1 overflow-y-auto p-5">
          ...
          <pre v-else class="whitespace-pre-wrap font-mono text-xs text-gray-800 dark:text-gray-200 leading-relaxed">{{ logModal.content }}</pre>
        </div>
      </div>
    </div>
  </Teleport>
