...
function startMerge(conflict) {
  mergeForms.value[conflict.id] = (conflict.memory_a?.content ?? '') + '\n\n' + (conflict.memory_b?.content ?? '')
}
  ...
  <div class="max-w-3xl mx-auto p-6 space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div class="flex items-center gap-3">
        ...
        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300">
    ...
    <!-- Add Memory Form -->
    <div v-if="showAddForm" class="bg-white dark:bg-gray-800 rounded-xl border border-indigo-300 dark:border-indigo-600 p-4 space-y-3">
    ...
    <div v-if="memoryStore.conflicts.length > 0" class="space-y-3">
      ...
      <div v-for="conflict in memoryStore.conflicts" :key="conflict.id" class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4 space-y-3">
        <p class="text-xs font-medium text-amber-700 dark:text-amber-300">Conflicting memories detected</p>
        <div class="grid grid-cols-2 gap-3">
        ...
        <div v-if="mergeForms[conflict.id] !== undefined" class="space-y-2">
          <label class="block text-xs font-medium text-gray-600 dark:text-gray-400">Merged content</label>
          <textarea
    ...
    <!-- Loading -->
    <div v-if="memoryStore.isLoading && memoryStore.memories.length === 0" class="flex items-center justify-center gap-2 py-12 text-gray-400 dark:text-gray-500 text-sm">
      <Loader2 class="w-5 h-5 animate-spin" /> Loading memories...
    </div>

    <!-- Empty state -->
    <div v-else-if="memoryStore.memories.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
    ...
    <div class="space-y-3">
      <div
        ...
        :class="{ 'border-indigo-300 dark:border-indigo-600 ring-1 ring-indigo-300 dark:ring-indigo-600': selectedIds.has(memory.id) }"
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0 space-y-2">
            ...
            <div v-if="editingId === memory.id" class="space-y-2">
              ...
              />
              <div class="flex items-center gap-3">
                <label class="text-xs text-gray-500 dark:text-gray-400">Importance: {{ editImportance }}</label>
                <input
            ...
            <!-- View mode -->
            <div v-else>
              <p class="text-sm text-gray-800 dark:text-gray-200 leading-relaxed">{{ memory.content }}</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
              <span
                ...
                :class="CATEGORY_COLORS[memory.category] ?? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'"
              >{{ memory.category }}</span>
              <span :class="importanceColor(memory.importance ?? 5)" class="flex items-center gap-0.5 font-medium">
    ...
    <!-- Clear All -->
    <div v-if="memoryStore.memories.length > 0" class="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
      ...
      <div v-else-if="clearConfirmStep === 1" class="flex items-center gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4">
        <AlertTriangle class="w-5 h-5 text-red-500 flex-shrink-0" />
        <div class="flex-1">
          <p class="text-sm font-semibold text-red-700 dark:text-red-300">This will permanently delete all memories.</p>
      ...
      <div v-else-if="clearConfirmStep === 2" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 space-y-3">
        <p class="text-sm font-semibold text-red-700 dark:text-red-300">
          Type <span class="font-mono bg-red-100 dark:bg-red-800/40 px-1 rounded">DELETE</span> to confirm
        </p>
        <input
        ...
        <div class="flex gap-2">
          <button
            type="button"
            class="px-3 py-1.5 rounded-lg text-sm bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600 transition"
            @click="clearConfirmStep = 0; clearTypeValue = ''"
          >Cancel</button>
          <button
            type="button"
            ...
            @click="memoryStore.bulkDestroy(memoryStore.memories.map((m) => m.id)).then(() => { clearConfirmStep = 0; clearTypeValue = ''; uiStore.addToast({ type: 'success', message: 'All memories cleared.' }) })"
          >
