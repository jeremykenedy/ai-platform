<script setup>
  ...
  async function handleConnect(integration) {
    if (integration.auth_type === 'oauth') {
      ...
      if (integration.oauth_url) {
        ...
        uiStore.addToast({
          type: 'error',
          message: 'OAuth URL not configured for this integration.',
        })
      }
      return
    }
  ...
  async function saveApiKey() {
    ...
    try {
      ...
      uiStore.addToast({
        type: 'success',
        message: `${apiKeyModal.integration.display_name} connected.`,
      })
    } catch (err) {
  ...
  async function testApiKey() {
    ...
    apiKeyModal.error = ''
    try {
      await integrationsStore.executeTool(apiKeyModal.integration.name, 'test', {
        api_key: apiKeyModal.key,
      })
...
<template>
  <div class="max-w-4xl mx-auto p-6 space-y-8">
    ...
    <div class="flex items-center gap-3">
      ...
      <span
        class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300"
      >
    ...
    <div
      ...
      class="flex items-center justify-center gap-2 py-16 text-gray-400 dark:text-gray-500 text-sm"
    >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <div class="flex items-start gap-3">
              <!-- Icon avatar -->
              <div
                class="w-10 h-10 rounded-xl bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center flex-shrink-0 text-indigo-700 dark:text-indigo-300 font-bold text-base select-none"
              >
            ...
            <div class="flex items-center gap-1.5 text-xs">
              <component
                ...
                :class="
                  integration.connected ? 'text-green-500' : 'text-gray-400 dark:text-gray-500'
                "
              />
              <span
                :class="
            ...
            <div
              ...
              class="flex items-start gap-1.5 text-xs text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg p-2"
            >
            ...
            <div
              v-if="disconnectConfirmName === integration.name"
              class="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg p-2 text-xs"
            >
              <span class="flex-1 text-red-700 dark:text-red-300"
                >Disconnect {{ integration.display_name }}?</span
              >
              <button
                type="button"
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6 space-y-4"
        >
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">
            Enter your API key for {{ apiKeyModal.integration?.display_name }}. This is stored
