...
import { Plus, Edit2, Trash2, ChevronDown, ChevronUp, Loader2, Star, Bot, X, Check, AlertTriangle } from 'lucide-vue-next'
  ...
  <div class="max-w-3xl mx-auto p-6 space-y-6">
    ...
    >
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" @click="closeCreate">
          <X class="w-5 h-5" />
        </button>
      </div>
    ...
    >
      ...
      <p class="text-gray-500 dark:text-gray-400 text-sm">No personas yet. Create one to customize AI behavior.</p>
    </div>

    <!-- Loading -->
    <div v-if="personasStore.isLoading" class="flex items-center gap-2 text-sm text-gray-400 dark:text-gray-500 py-8 justify-center">
      <Loader2 class="w-5 h-5 animate-spin" /> Loading personas...
    </div>

    <!-- Persona Cards -->
    <div v-for="persona in personasStore.sortedPersonas" :key="persona.id" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      <!-- Card header -->
      <div class="flex items-start gap-3 p-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ persona.name }}</h3>
            <span
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            {{ (persona.system_prompt ?? '').slice(0, 100) }}{{ (persona.system_prompt ?? '').length > 100 ? '...' : '' }}
          </p>
      ...
      >
        ...
        <span class="flex-1 text-red-700 dark:text-red-300">Delete "{{ persona.name }}"? This cannot be undone.</span>
        <button
          type="button"
      ...
      <!-- Edit Form -->
      <div v-if="expandedId === persona.id && editForms[persona.id]" class="border-t border-gray-100 dark:border-gray-700 p-4 space-y-4">
        <PersonaFormFields :form="editForms[persona.id]" :errors="editForms[persona.id].errors" :models="modelsStore.models" />
...
const PersonaFormFields = defineComponent({
  ...
  setup(props) {
    return () => {
      ...
      const label = (text) =>
        h('label', { class: 'block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1' }, text)
      ...
      return h('div', { class: 'space-y-4' }, [
        ...
        h('div', [
          h('div', { class: 'flex items-center justify-between mb-1' }, [
            label('Temperature'),
            h('span', { class: 'text-xs font-mono text-gray-500 dark:text-gray-400' }, f.temperature.toFixed(2)),
          ]),
          h('input', {
            type: 'range',
            min: 0,
          ...
          h('div', { class: 'flex justify-between text-xs text-gray-400 dark:text-gray-500 mt-0.5' }, [
            h('span', '0 (precise)'),
            h('span', '2 (creative)'),
          ]),
        ]),
        ...
        h('div', [
          h('div', { class: 'flex items-center justify-between mb-1' }, [
            label('Top P'),
            h('span', { class: 'text-xs font-mono text-gray-500 dark:text-gray-400' }, f.top_p.toFixed(2)),
          ]),
          h('input', {
            type: 'range',
            min: 0,
        ...
        h('div', { class: 'grid grid-cols-2 gap-4' }, [
          ...
          h('div', [
            h('div', { class: 'flex items-center justify-between mb-1' }, [
              ...
              h('span', { class: 'text-xs font-mono text-gray-500 dark:text-gray-400' }, f.repeat_penalty.toFixed(2)),
            ]),
            h('input', {
              type: 'range',
              min: 0.5,
        ...
        h('div', { class: 'flex items-center gap-3' }, [
          ...
          h('div', { class: 'flex items-center gap-1.5 text-sm font-medium text-gray-700 dark:text-gray-300' }, [
            h(Star, { class: 'w-4 h-4 text-amber-400' }),
